  ...
  const paginate = (newDirection: number) => {
    if (isExpanded) {
      ...
      setTimeout(() => {
        ...
        setCurrentIndex((prev) => (prev + newDirection + totalPages) % totalPages);
      }, 100);
    } else {
      setDirection(newDirection);
      setCurrentIndex((prev) => (prev + newDirection + totalPages) % totalPages);
              ...
              >
                ...
                >
                  {visibleMaterials.map((material, i) => {
                    const isCardExpanded = expandedCardIndex === i && isExpanded;
            ...
            <div className="flex space-x-3">
              {Array.from({ length: totalPages }).map((_, i) => (
                <motion.button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors ${i === currentIndex ? 'bg-gray-800' : 'bg-gray-300 hover:bg-gray-400'}`}
